...
> Detailed student-facing guides : [docs/STUDENT-INSTALL-EN.md](./docs/STUDENT-INSTALL-EN.md) (English) and [docs/STUDENT-INSTALL-FR.md](./docs/STUDENT-INSTALL-FR.md) (français). Send students to those, not to this page.
...
Both scripts share the exact same behavior :

- check `docker`, `docker compose v2`, `openssl` (PowerShell falls back to .NET RNG if OpenSSL is missing)
- generate `TEACHER_ADMIN_TOKEN` and `DASHBOARD_TEACHER_TOKEN` (32 chars hex each) and write `docker/.env` from `.env.example` if it does not exist yet
- preserve any token already present in `docker/.env` (idempotent — re-running them does not rotate keys)
- run `docker compose --env-file .env up -d --build`
- poll `http://127.0.0.1:3000/` and `http://127.0.0.1:5000/api/health` until both answer
- print both teacher tokens and the student / teacher URLs at the end

Other modes :

| bash | PowerShell | Effect |
|---|---|---|
| `./scripts/install-student.sh`         | `.\scripts\install-student.ps1`         | interactive, asks for cohort_id |
| `./scripts/install-student.sh -y`      | `.\scripts\install-student.ps1 -Yes`    | non-interactive, default cohort |
| `./scripts/install-student.sh --reset` | `.\scripts\install-student.ps1 -Reset`  | `docker compose down -v` + clean reinstall |

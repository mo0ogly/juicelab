"""Phase 3 Task 1 : mode toggle live|analyse."""
...
def test_default_mode_is_analyse(client):
    r = client.get("/dashboard?cohort=demo", headers=AUTH)
    assert r.status_code == 200
    body = r.data.decode()
    assert 'class="mode-analyse"' in body


def test_mode_live_renders(client):
    r = client.get("/dashboard?cohort=demo&mode=live", headers=AUTH)
    assert r.status_code == 200
    body = r.data.decode()
    assert 'class="mode-live"' in body

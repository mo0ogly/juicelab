...
| 18 | `908d7e5` | **Docker hardening + hadolint gate**. `Dockerfile.dashboard` migre de `requirements.txt` -> `requirements.lock.txt` avec `pip install --require-hashes` (sha256 attestation au build). Nouveau `HEALTHCHECK` Docker-level dans les 2 Dockerfiles (dashboard via urllib.request /api/health, juicelab via http.get /rest/admin/application-version, --start-period 60s pour Angular bootstrap). `docker-compose.yml` durcit le service dashboard : `security_opt: no-new-privileges:true` (kill SUID/CAP gain via execve, mitige escalade) + `cap_drop: ALL` (zero Linux capability ; pure-Python+SQLite ne necessite ni raw socket ni chown ni mknod). SEC-15 nouveau gate : hadolint v2.12.0 sur les 2 Dockerfiles, config `.hadolint.yaml` ignore les warnings pedantiques (DL3008 apt pin, DL3059 RUN consol, DL3003 WORKDIR, DL4006 pipefail), threshold=error. Status : 0 errors, 0 warnings (apres filtrage config). CI : install hadolint binary v2.12.0 dans le job `dashboard-security-recette`. Recette **14 -> 15 checks**. 177/177 pytest PASS + 71/71 bash recettes PASS + 15/15 security recette PASS. |
| 17 | `20b20ad` | **HSTS opt-in + SECURITY_POSTURE.md consolidation**. Nouveau header `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload` (2 ans, eligible HSTS preload list) emis **uniquement** quand `DASHBOARD_HTTPS=true`. Defense vs SSL strip / downgrade attack. Refus de l'emission par defaut empeche bricker un deploy local en HTTP (HSTS = sticky, irreversible cote browser). `+5 tests test_hsts_header.py` : absent quand var unset/false/invalid, present + max-age>=1an + includeSubDomains + preload quand `=true`, case-insensitive `TRUE`. Nouveau `docs/SECURITY_POSTURE.md` (180 lignes) : single-page summary OWASP-ready de tous les controles installes (SAST/SCA/secrets/network/anti-tampering/operations/DAST/testing/supply-chain/CI), trajectoire 17 cycles (72.75 -> 99.99 plateau), residual risks documentes, instructions `how to verify`. Cible : audit ext, procurement questionnaires, soumission upstream OWASP. 177/177 pytest PASS + 71/71 bash recettes PASS + 14/14 security recette PASS. |
| 16 | `6da5665` | **Dependabot + LICENSES.md auto-gen + drift gate**. Nouveau `.github/dependabot.yml` : 3 ecosystemes (pip pour `dashboard/`, github-actions pour `.github/`, docker pour `docker/`). Hebdo lundi 04h30 Europe/Paris pour pip, mensuel pour actions+docker. Groupes : `test-deps` (pytest*, coverage), `flask-stack` (Flask/Jinja/Werkzeug/MarkupSafe/itsdangerous/click/blinker) -- evite la dispersion en plusieurs PR pour des deps couples. Ignore major bumps (semver-major). Labels auto : `dependencies + python + security`. Nouveau `scripts/gen_licenses.sh` + `dashboard/LICENSES.md` : auto-generation via `pip-licenses --packages $(uniq lockfile)` + dedup wheels Python 3.11/3.13. SEC-14 nouveau gate recette : regenere LICENSES.md et diff vs commit, bloque si drift (force update apres bump de lockfile). Recette **13 -> 14 checks**. 172/172 pytest PASS + 71/71 bash recettes PASS + 14/14 security recette PASS. |
